...
class Assignment < ActiveRecord::Base
	...
	def create_contributions
		self.students.each do |student|
			Contribution.create( user_id: student , assignment_id: self )
		end 
	end 
	...
	def check_for_pull_requests
		...
		pull_requests.each do |pr|
			binding.pry
			user_id = User.where(github_login: (pr["user"]["login"].to_i).first)
			contribution = Contribution.where( assignment_id: self, user_id: user)
